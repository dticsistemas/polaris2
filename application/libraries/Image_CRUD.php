...
class Image_CRUD {
	...
	protected $ci = null;
	protected $thumbnail_prefix = 'thumbs/';
	protected $thumbnail_prefix_small = 'small/';//---ad_mod
	protected $thumbnail_prefix_medium = 'medium/';//--add_mod
    ...
    protected function _delete_file($id)
    {
    	...
    	//--borrando las miniaturas creadas--
    	unlink( $this->image_path.'/'.$this->thumbnail_prefix_small.$result->{$this->url_field} );
        unlink( $this->image_path.'/'.$this->thumbnail_prefix_medium.$result->{$this->url_field} );
    ...
    protected function _get_photos($relation_value = null)
    {
    	...
    	foreach($results as $num => $row)
    	{
			if (!file_exists($this->image_path.'/'.$this->thumbnail_prefix.$row->{$this->url_field})) {
				$this->_create_thumbnail($this->image_path.'/'.$row->{$this->url_field},$row->{$this->url_field});
			}
	...
	{
		//---para crear imagen thumbs
		$this->image_moo
			->load($image_path)
			->resize_crop(120,90)
			->save($this->image_path.'/'.$this->thumbnail_prefix.$thumbnail_path,true);
		//---para crear imagen small
		$this->image_moo
			->load($image_path)
			->resize(300,225)
			->save($this->image_path.'/'.$this->thumbnail_prefix_small.$thumbnail_path,true);
			//---para crear imagen medium
		$this->image_moo
			->load($image_path)
			->resize(500,375)
			->save($this->image_path.'/'.$this->thumbnail_prefix_medium.$thumbnail_path,true);
	}
	...
	protected function getState()
	{
		...
		elseif(isset($rsegments_array[3]) && $rsegments_array[3] == 'upload_file')
		{
			#region Just rename my file
				...
				//----quitando el renombrado del archivo no funciono el reorte del
		...
		if(!empty($state_info))
		{
			switch ($state_info->name) {
				...
				case 'upload_file':
					...
					if ($file_name !== false) {
						$this->_create_thumbnail( $this->image_path.'/'.$file_name , $file_name );
...
class ImageUploadHandler
{
	...
	private function trim_file_name($name, $type) {
		...
		//PERFECTO AQui Renombra al nuevo archivo
		$file_name = substr(uniqid(),-6).substr(preg_replace("/([^a-zA-Z0-9\.\-\_]+?){1}/i", '-', $file_name),-4);
